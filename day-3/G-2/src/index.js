...
const EventExample = () => {
  const DontClick = () => {
    console.log("I SAID DO NOT CLICK!");
  };
  const dontType = (e) => {
    console.log(e.target.style);
    console.log(e.target.value);
    alert(`i said don't type! " ${e.target.value} " do not type this again`);
    let DATA__AN = [];
    DATA__AN.push(e.target.value);
    localStorage.setItem("reactData", JSON.stringify(DATA__AN));
  };
  const dontSubmitMe = (e) => {
    e.preventDefault();
    console.log(
      `are you talking to me? did you just said " ${JSON.parse(
        localStorage.getItem("reactData")
      )} "`
    );
  };
  return (
    <section>
      <form onSubmit={dontSubmitMe}>
        <h2 style={{ margin: " 0  0  0 1rem" }}>typical form</h2>
        <input
          onChange={dontType}
          type="text"
          name="text-Y"
      ...
      <div>
        <button
          ...
          onClick={(e) => {
            e.preventDefault();
            console.log(
              `are you talking to me? did you just click on me and said : " ${JSON.parse(
                localStorage.getItem("reactData")
              )} "`
            );
          }}
      ...
      <div> YOUR last DATA:{JSON.parse(localStorage.getItem("reactData"))}</div>
    </section>
  );
};
const JustPrint = () => {
  // const DontClick = document.getElementById("dontClick");
  // daddy.addEventListener("click", function () {
  //   console.log("dont click here again");
  // });
  return <EventExample />;
};
